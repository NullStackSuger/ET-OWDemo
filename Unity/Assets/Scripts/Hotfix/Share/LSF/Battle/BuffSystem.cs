namespace ET
{
    ...
    public static partial class BuffSystem
    {
        ...
        private static void Awake(this Buff self, int configId)
        {
            ...
            // 发布事件: 受到Buff
            EventSystem.Instance.Publish(self.IScene as LSWorld, new UnitUseBuff() { Owner = player, Buff = self });
        }
        ...
        private static void Destroy(this Buff self)
        {
            // 发布事件: 销毁技能
            LSUnit player = self.GetParent<BuffComponent>().GetParent<LSUnit>();
            EventSystem.Instance.Publish(self.IScene as LSWorld, new UnitRemoveBuff() { Owner = player, Buff = self });
        }
    }
}